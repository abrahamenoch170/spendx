...
export const EmergencyInfoCard = () => {
  const { selectedEmergency, setSelectedEmergency, setActionState } = useMapStore();

  if (!selectedEmergency) return null;

  const handleDirections = () => {
    // Logic to trigger route preview
    setActionState('routePreview', true);
    // Zoom/highlight logic would be handled by the map component listening to state
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        animate={{ y: 0, opacity: 1, transition: { type: 'spring', damping: 20, stiffness: 100 } }}
      ...
      >
        ...
        <button 
          ...
          className="w-full py-3 bg-[var(--acid-lime)] text-black font-bold rounded-xl"
        >
          Directions
        </button>
      </motion.div>
