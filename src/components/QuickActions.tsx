...
export function QuickActions() {
  ...
  return (
    <motion.div 
      ...
      className="fixed bottom-6 left-1/2 -translate-x-1/2 md:left-auto md:translate-x-0 md:right-8 z-50 glass-panel rounded-full px-4 py-3 flex items-center gap-4 border border-white/20 shadow-[0_10px_40px_rgba(0,0,0,0.5)]"
    >
      {actions.map((action, i) => (
        <motion.button
          ...
          className="flex flex-col md:flex-row items-center gap-2 text-white/70 hover:text-white transition-colors group min-w-[44px] min-h-[44px] justify-center"
        >
          <div 
