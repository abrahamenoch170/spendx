...
export function NavigationMenu() {
  ...
  return (
    <>
      ...
      <button
        onClick={toggleMenu}
        className="fixed top-6 right-6 z-[100] w-12 h-12 flex flex-col items-center justify-center gap-1.5 rounded-full bg-[var(--bg-color)]/50 backdrop-blur-md border border-[var(--border-color)] transition-all hover:scale-105"
        style={{ boxShadow: `0 0 15px ${activeColor}` }}
      >
        <motion.div
          animate={isOpen ? { rotate: 45, y: 8 } : { rotate: 0, y: 0 }}
          transition={{ duration: 0.35, ease: springEasing }}
          className="w-6 h-0.5 bg-[var(--text-primary)] rounded-full will-change-transform"
        />
        <motion.div
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            style={{ backgroundColor: activeColor, opacity: 0.95, backdropFilter: 'blur(12px)', willChange: 'clip-path' }}
